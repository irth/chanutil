...
func TestRequestCanOnlyBeAnsweredOnce(t *testing.T) {
	...
	reqCh := make(chanutil.RequestChannel[int, int], 1) // queue to avoid blocking
	defer close(reqCh)
	...
	checkErrors := func(errCh chan error) {
		...
		require.NoError(t, err)
		require.NotNil(t, testErr)
		require.ErrorIs(t, *testErr, chanutil.ErrResultSentTwice)
	}

	res, err := reqCh.Call(ctx, 0)
	checkVal(testVal1, res, err)
	checkErrors(errCh)

	_, err = reqCh.Call(ctx, 1)
	require.ErrorIs(t, err, testErr1)
	checkErrors(errCh)

	res, err = reqCh.Call(ctx, 2)
	checkVal(testVal1, res, err)
	checkErrors(errCh)

	_, err = reqCh.Call(ctx, 3)
	require.ErrorIs(t, err, testErr1)
	checkErrors(errCh)
}
